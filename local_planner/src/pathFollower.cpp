...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subOdom = nh.subscribe<nav_msgs::Odometry> ("/wamv/robot_localization/odometry/filtered", 5, odomHandler);
  ...
  // cmd_vel.header.frame_id = "hunter_se/base_link";
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      //第一次的时候，pubSkipCount = 0； 第二次及其以后pubSkipCount = 1.
      pubSkipCount--;
      if (pubSkipCount < 0) {
        // cmd_vel.header.stamp = ros::Time().fromSec(odomTime);
        if (fabs(vehicleSpeed) <= maxAccel / 100.0) {
          
          // std::cout << "maxAccel / 100.0 = "<< maxAccel / 100.0 <<std::endl;
        ...
        // cmd_vel.linear.x = 10*vehicleSpeed;
