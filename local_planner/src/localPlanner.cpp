...
void odometryHandler(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  vehiclePitch = pitch;
  vehicleYaw = yaw;
  //将offsetX 或Y 计算出相对yaw的长度后，就是相对于绝对坐标系的值。这里可以相当与车辆的行进中心和传感器的偏移。
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloud2)
{
  if (!useTerrainAnalysis) {
    laserCloud->clear();
    //将ros消息中的点云转换成pcl库中的点云，其中，laserCloud2是ros消息的点云。
...
void terrainCloudHandler(const sensor_msgs::PointCloud2ConstPtr& terrainCloud2)
{
  if (useTerrainAnalysis) {
    ...
    for (int i = 0; i < terrainCloudSize; i++) {
      ...
      float dis = sqrt((pointX - vehicleX) * (pointX - vehicleX) + (pointY - vehicleY) * (pointY - vehicleY));
      //dis小于某个阈值and强度大于某个阈值。或者使用了usuCost标志，标志全都存下去。
...
void boundaryHandler(const geometry_msgs::PolygonStamped::ConstPtr& boundary)
{
  ...
  //需要将point1和point2进行计算或者比较，因此采用如此逻辑。可以学学习。
...
int main(int argc, char** argv)
{
  ...
  //joy数据类型，用于表示手柄的状态，包括手柄的按钮和强度信息。
  ...
  while (status) {
    ros::spinOnce();
    //存在这两种点云数据的更新。laserDwzFilter 和 terrainDwzFilte 那么进入主循环。
    if (newLaserCloud || newTerrainCloud) {
      ...
      //plannerCloudCrop 中存入plannerCloud的内容，前提是经过坐标变换转换成相对于车的中心的坐标值。
      pcl::PointXYZI point;
      ...
      //将边缘的所有点存入plannerCloud中，这里的坐标转换规则和上面的一样。
      ...
      //将新增障碍物的所有点存入plannerCloud中，同理，转换规则是一样的。
      int addedObstaclesSize = addedObstacles->points.size();
      for (int i = 0; i < addedObstaclesSize; i++) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        //清空路径列表及其penalty值。一共36条路径，即6*6；
        for (int i = 0; i < 36 * pathNum; i++) {
          clearPathList[i] = 0;
          pathPenaltyList[i] = 0;
        }
        for (int i = 0; i < 36 * groupNum; i++) {
          //表示proability的可能通过的线数的条数。最大值为49。
        ...
        //对所有的plannerCloudCrop进行判断，判断36个方向是否与之发生碰撞或者存在危险。不危险的留下，危险的清除，这样标记出来所有的路径，有利于将路径显示化的方便操作。
        for (int i = 0; i < plannerCloudCropSize; i++) {
          //按照pathScale大小进行计算。pathScale = 1，代表轨迹库未原来轨迹库。
          ...
          if (dis < pathRange / pathScale && (dis <= (relativeGoalDis + goalClearRange) / pathScale || !pathCropByGoal) && checkObstacle) {
            //遍历-180 ，并按照逆时针进行遍历360度。
            for (int rotDir = 0; rotDir < 36; rotDir++) {
              float rotAng = (10.0 * rotDir - 180.0) * PI / 180;
              //求某参数相对于目标角度的差值，并转换成180角以内的角。
              ...
              if (indX >= 0 && indX < gridVoxelNumX && indY >= 0 && indY < gridVoxelNumY){
                ...
                for (int j = 0; j < blockedPathByVoxelNum; j++) {
                  ...
                  if (h > obstacleHeightThre || !useTerrainAnalysis) {
                    ...
                    //这个pathPenaltyList列表里面分别计算可以存在的路径的代价值。 pathPenaltyList的存储规则为，先计算 pathNum * rotDir， 表示某一组角度的所有路径。
          ...
          //根据需要更新minObsAngCCW minObsAngCW ，具体含义未知。
        ...
        //选取剩余可行路径最多的路径组合，即角度组合 + 开始段组合。形成唯一选择。
        float maxScore = 0;
